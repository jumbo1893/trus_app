...
class _RunAndKickAnimationScreenState extends State<RunAndKickAnimationScreen> with TickerProviderStateMixin {
  ...
  void setShootingPlayerAnimation() {
    ...
    _rightThighAngle = TweenSequence(
        BodyTweenSequenceComponents(
            [-20, -10, 5, 30, 60], 90).getTweenComponents()
    ).animate(_controller);
  ...
  void setRunningPlayerAnimation() {
    ...
    _rightThighAngle = TweenSequence(
        BodyTweenSequenceComponents(
            [firstRightLegAngle, 60, 10, -5, -10], -20).getTweenComponents()
    ).animate(_bodyController);
    _leftCalfAngle = TweenSequence(
    ...
    _bodyY1 = TweenSequence(
        BodyTweenSequenceComponents([0, 10, 0, -5, -15], 0)
            .getTweenComponents()
    ).animate(_bodyController);
  }
  ...
  void setDrinkingPlayer() {
    ...
    _rightThighAngle = TweenSequence(
        BodyTweenSequenceComponents(
            [-20,  10, 10, 10, 10], firstRightLegAngle).getTweenComponents()
    ).animate(_controller);
    _leftCalfAngle = TweenSequence(
    ...
    _rightCalfAngle = TweenSequence(
        BodyTweenSequenceComponents([-60, 0, 0, 0.1, 0.1], 0.1) //větší než 0 = nebude pěna
            .getTweenComponents()
    ).animate(_controller);
    _bodyY1 = TweenSequence(
        BodyTweenSequenceComponents([0, 0, 0.1, 0.2, 0], 0) //desetinný čárky určujou kdy bude pivo v ruce
            .getTweenComponents()
    ).animate(_controller);
  }
  ...
  void setCelebratingPlayerAnimation() {
    ...
    _bodyY1 = TweenSequence(
        BodyTweenSequenceComponents([0, -15, 0, -15, 0], 0)
            .getTweenComponents()
    ).animate(_controller);
  }
...
class _RunAndKickPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double ballY = figure.ballComponents.dy *
        (ballJumpingAnimationValue * 0.1 + 0.9);
    if(status == LoadingAnimationStatus.playerRunning) {
      ...
      drawMovingFigure(figure, canvas, linePaint, headRadius);
      drawMovingTable(figure, canvas, goalPaint, bodyX1, tableX, tableLength, tableHeight, size);
      drawMovingBeer(canvas, goalPaint, tableX, tableLength, tableHeight, bodyX1, size);
    }
    else if(status == LoadingAnimationStatus.playerDrinking) {
      drawMovingFigure(figure, canvas, linePaint, headRadius);
      drawStandingTable(canvas, goalPaint, tableX, tableLength, tableHeight, size);
      drawDrinkingBeer(figure, canvas, goalPaint, tableX, tableLength, tableHeight, size);
    }
    else if(status == LoadingAnimationStatus.tableLeaving) {
      drawMovingFigure(figure, canvas, linePaint, headRadius);
      drawMovingTableAway(canvas, goalPaint, tableX, tableLength, tableHeight, size);
      drawMovingBeerAway(canvas, goalPaint, tableX, tableLength, tableHeight, size);
    }
  }
  ...
  void drawMovingFigure(FigureLinesCalculator figure, Canvas canvas, Paint linePaint, double headRadius) {
    canvas.drawLine(
        figure.bodyComponents[0], figure.bodyComponents[1], linePaint);
    canvas.drawLine(
        figure.leftArmComponents[0], figure.leftArmComponents[1], linePaint);
    canvas.drawLine(
        figure.rightArmComponents[0], figure.rightArmComponents[1],
        linePaint);
    ...
    canvas.drawLine(
        figure.leftCalfComponents[0], figure.leftCalfComponents[1],
        linePaint);
  ...
  void drawMovingBall(FigureLinesCalculator figure, Canvas canvas, Paint linePaint, double ballX, double ballY, double ballRadius) {
    canvas.drawCircle(Offset(ballX, ballY), ballRadius, linePaint);
  }

  void drawStandingBall(FigureLinesCalculator figure, Canvas canvas, Paint linePaint, double ballRadius) {
    canvas.drawCircle(Offset(figure.ballComponents.dx, figure.ballComponents.dy), ballRadius, linePaint);
  }

  void drawFlyingBall(FigureLinesCalculator figure, Canvas canvas, Paint linePaint, double bodyX1, double ballRadius, Size size) {
    final double goalX = bodyX1 + size.width/2 +size.width/1.65;
    final double ballX = figure.ballComponents.dx +
        (goalX - figure.ballComponents.dx) * ballFlyingAnimationValue;
    final double ballY = figure.ballComponents.dy + (0+ ballRadius*3 - figure.ballComponents.dy) *
        (ballFlyingAnimationValue);

    canvas.drawCircle(Offset(ballX, ballY), ballRadius, linePaint);
  }

  void drawFallingBall(FigureLinesCalculator figure, Canvas canvas, Paint linePaint, double bodyX1, double ballRadius, Size size) {
    ...
    final double ballX = upperRightGoalX + (upperRightGoalX-bottomRightGoalX)*kickAnimationValue;
    final double ballY = (size.height-ballRadius) + (ballRadius*3 - (size.height-ballRadius)) *
        (ballFallingAnimationValue);
    canvas.drawCircle(Offset(ballX, ballY), ballRadius, linePaint);
  }

  void drawMovingBallAway(FigureLinesCalculator figure, Canvas canvas, Paint linePaint, double bodyX1, double ballRadius, Size size) {
    final double goalX = bodyX1 + size.width/2;
    final double upperRightGoalX = goalX +size.width/1.3;
    final double leftBottomPostX = goalX+ size.width/1.85;
    final double lineFromLeftToRightUpper = upperRightGoalX-leftBottomPostX; //první poloha - stojí pod horním rohem
    final double ballX = (0-size.width) + (goalX+size.width/1.85 + size.width) * (kickAnimationValue-1).abs();
    final double ballY = size.height-ballRadius;
    canvas.drawCircle(Offset(ballX+lineFromLeftToRightUpper, ballY), ballRadius, linePaint);
  }

  void drawMovingGoal(FigureLinesCalculator figure, Canvas canvas, Paint goalPaint, double bodyX1, double ballRadius, Size size) {
    final double goalX = bodyX1 +
        (size.width - bodyX1) * kickAnimationValue + size.width/2;
    Offset leftPostOffset = Offset(goalX+size.width/1.85, size.height);
    Offset rightPostOffset = Offset(goalX+size.width/1.85+size.width/3, size.height);
    GoalPaint goal = GoalPaint(leftPostOffset, rightPostOffset, size.height-ballRadius);
    canvas.drawPath(goal.returnGoalPaint(), goalPaint);
  }

  void drawStandingGoal(FigureLinesCalculator figure, Canvas canvas, Paint goalPaint, double bodyX1, double ballRadius, Size size) {
    final double goalX = bodyX1 + size.width/2;
    Offset leftPostOffset = Offset(goalX+size.width/1.85, size.height);
    Offset rightPostOffset = Offset(goalX+size.width/1.85+size.width/3, size.height);
    GoalPaint goal = GoalPaint(leftPostOffset, rightPostOffset, size.height-ballRadius);
  ...
  void drawMovingGoalAway(FigureLinesCalculator figure, Canvas canvas, Paint goalPaint, double bodyX1, double ballRadius, Size size) {
    final double goalX = bodyX1 + size.width/2;
    final double lineFromLeftToRight = size.width/3;
    final double leftGoalX = (0-size.width) + (goalX+size.width/1.85 + size.width) * (kickAnimationValue-1).abs();
    Offset leftPostOffset = Offset(leftGoalX, size.height);
    Offset rightPostOffset = Offset(leftGoalX+lineFromLeftToRight, size.height);
    GoalPaint goal = GoalPaint(leftPostOffset, rightPostOffset, size.height-ballRadius);
    canvas.drawPath(goal.returnGoalPaint(), goalPaint);
  }

  void drawMovingTable(FigureLinesCalculator figure, Canvas canvas, Paint goalPaint, double bodyX1, double tableX, double tableLength, double tableHeight, Size size) {
    final double tableX1 = tableX +
        (size.width - bodyX1) * kickAnimationValue;
  ...
  void drawStandingTable(Canvas canvas, Paint goalPaint, double tableX, double tableLength,double tableHeight, Size size) {
  ...
  void drawMovingTableAway(Canvas canvas, Paint goalPaint, double tableX, double tableLength,double tableHeight, Size size) {
    double leftTableX = (0-size.width) + (tableX+size.width) * kickAnimationValue;
  ...
  void drawMovingBeerAway(Canvas canvas, Paint goalPaint, double tableX, double tableLength,double tableHeight, Size size) {
    double leftTableX = (0-size.width) + (tableX+size.width) * kickAnimationValue;
  ...
  void drawMovingBeer(Canvas canvas, Paint goalPaint, double tableX, double tableLength, double tableHeight, double bodyX1, Size size) {
    final double beerX = tableX +
        (size.width - bodyX1) * kickAnimationValue;
  ...
  void drawStandingBeer(Canvas canvas, Paint goalPaint, double tableX, double tableLength,double tableHeight, Size size, bool foam) {
  ...
  void drawDrinkingBeer(FigureLinesCalculator figure, Canvas canvas, Paint goalPaint, double tableX, double tableLength, double tableHeight, Size size) {
    if(figure.bodyY1 > 0) {
      ...
      drawStandingBeer(
          canvas, goalPaint, tableX, tableLength, tableHeight, size, figure.rightCalfAngle <= 0);
    }
  }

  void drawStandingFigure(FigureLinesCalculator figure, Canvas canvas, Paint linePaint, double headRadius) {
    canvas.drawLine(
        figure.bodyComponents[0], figure.bodyComponents[1], linePaint);
    canvas.drawLine(
        figure.leftArmComponents[0], figure.leftArmComponents[1], linePaint);
